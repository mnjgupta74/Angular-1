...
@Component({
  selector: 'app-approvebills-auther',
  templateUrl: './approvebills-auther.component.html',
  styleUrls: ['./approvebills-auther.component.scss'],
  animations: [
    trigger('state', [
      state(
        'visible',
...
export class ApprovebillsAutherComponent implements OnInit {
  ...
  this.ApiMethods.postresultservice(
    ...
    (response:any) => {
        ...
        if (response.result.out_str == "1" ) {
          ...
          this.loader.setLoading(false);
          this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
            this.router.navigate(['/ApprovebyAuth']);
          });
        } else {
