...
@Component({
  selector: 'app-approvebills-process',
  templateUrl: './approvebills-process.component.html',
  styleUrls: ['./approvebills-process.component.scss'],
  animations: [
    trigger('state', [
      state(
        'visible',
...
export class ApprovebillsProcessComponent implements OnInit {
        ...
        (response:any) => {
            ...
            if (response.result && response?.result?.Object1.length > 0 || response?.result?.Object2?.length >0 ) {
              ...
              this.tableDataSource.data = response?.result?.Object1;
                ...
                this.fileSelectionList=response?.result?.Object2;
  ...
  this.ApiMethods.postresultservice(
    ...
    (response:any) => {
        ...
        if (response.result.out_str == "1" ) {
          ...
          this.loader.setLoading(false);
          this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
            this.router.navigate(['/ApprovebyProcess']);
          });
        } else {
