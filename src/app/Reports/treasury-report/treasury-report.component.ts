...
export class TreasuryReportComponent implements OnInit {
  @ViewChild('test', { static: false }) el!: ElementRef;
  filename = "TreasuryLORReport.xlsx";
  ...
  ngOnInit(): void {
    ...
    this.TreasReportForm = new FormGroup({
  ...
  getTreasuryList() {
    ...
    this.ApiMethods.getservice(this.ApiService.treasuryList).subscribe((resp: any) => {
      ...
      if (resp.result && resp.result.length > 0) {
        ...
        this.Treasuryoptions = this.TreasReportForm.controls['treasuryval'].valueChanges.pipe(
          startWith(''),
          map((value: any) => {
        ...
        this.TreasReportForm.patchValue({
  ...
  onSearch() {
    ...
    let Trescode = this.TreasReportForm.controls['treasuryval'].value;
    let Date1 = this.TreasReportForm.controls['fromDate'].value;
    let fDate = new DatePipe('en-US').transform(Date1, 'yyyy-MM-dd');
    let Date2 = this.TreasReportForm.controls['toDate'].value;
    ...
    this.ApiMethods.postresultservice(this.ApiService.Getlorlist, body).subscribe((resp: any) => {
      console.log("After_Calling_API_loR_Result", resp);
...
exportexcel(json: any[], excelFileName: string): void {
  var Heading: any = [
    [' Sr No.','PD DIV DTLS',' GRP','Commission','Group Head',' Budget Head','Div Code', 'Discription of Budget Head No.', 'Amount (in Rs.)', 'Challan Amount', 'Total Challan', ' Div Challan Amount', 'Head Range', 'BT ADJ Amount', 'Div BT ADJ Amount', 'Gross Amount ']
  ];
  let arr: any[] = []
    json.forEach((item: any, index: any) => {
      let a = {
        " Sr No.": index + 1,
        "PD DIV DTLS": item.CDE_REFNO,
        "Cde_Reference No.": item.PD_DIV_DTLS,
        "GRP": item.GRP,
        "Commission": item.Commission,
        "Group Head": item.GroupHead,
        "Budget Head": item.BHName,
        "Div Code": item.DivCode,
        "Discription of Budget Head No.": item.MajorHeadNameHindi,
        "Amount (in Rs.)": item.ReceiptAmt,
        "Challan Amount": item.ChallanAmt,
        "Total Challan": item.TotalChallan,
        " Div Challan Amount": item.DivChallanAmt,
        "Head Range": item.HeadRange,
        "BT ADJ Amount": item.BTADJAmt,
        "Div BT ADJ Amount": item.DivBTADJAmt,
...
saveAsExcelFile(buffer: any, fileName: string): void {
  ...
  exportToPdf() {
    ...
    // doc.text("https://rajkosh.rajasthan.gov.in", 10, 200);
      doc.save("TreasuryLORReport.pdf");
      this.loader.setLoading(false);
    }, 500);
